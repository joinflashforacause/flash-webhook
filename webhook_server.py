...
def send_template(to, name, amount, date, template_name=None, media_type=None, media_url=None):
    """Send a template. If media_type+media_url given, the template must have
    an image/video header. Otherwise uses the text-header Telugu template."""
    ...
    return http_requests.post(
        ...
        json={"messaging_product": "whatsapp", "to": to, "type": "template",
              "template": {"name": tname, "language": {"code": TEMPLATE_LANGUAGE},
...
def bulk_worker(rows, cap, delay, template_name, media_type, media_url):
    global bulk_state
...
def api_bulk_start():
    ...
    if media_type and not media_url:
        return jsonify({"error": "Media type chosen but no media URL given"}), 400
    ...
    threading.Thread(target=bulk_worker,
                     args=(rows, cap, 1.5, template_name, media_type, media_url),
                     daemon=True).start()
...
రాజేష్ కుమార్,9123456789,"1,116",29 జూన్ 2026'></textarea>
    ...
    <div class="row2">
      ...
      <div>
        ...
        <select id="bulkMediaType" onchange="toggleMediaFields()">
          <option value="">Text template (Telugu thank-you) — default</option>
          <option value="image">Image-header template</option>
          <option value="video">Video-header template</option>
        </select>
      </div>
    </div>
    ...
    <div id="mediaFields" style="display:none">
      <div class="row2">
        <div>
          <label>Media template name (as approved in WhatsApp Manager)</label>
          <input id="bulkTemplateName" placeholder="donor_thank_you_image_te">
        </div>
        <div>
      ...
      <div class="mediahint">
        Requires a template with an IMAGE or VIDEO header approved in WhatsApp Manager
        (your current Telugu template has a text header — create a media one first).
        The URL must be publicly downloadable. Video: MP4, under 16MB.
      </div>
    </div>
...
function toggleMediaFields() {
  const v = document.getElementById('bulkMediaType').value;
  document.getElementById('mediaFields').style.display = v ? 'block' : 'none';
}
...
async function startBulk() {
  ...
  const templateName = mediaType ? document.getElementById('bulkTemplateName').value.trim() : '';
  const mediaUrl = mediaType ? document.getElementById('bulkMediaUrl').value.trim() : '';
  if (!csv) { alert('Paste your CSV rows first.'); return; }
  if (mediaType && (!templateName || !mediaUrl)) { alert('Media template name and URL are required for image/video sends.'); return; }
  if (!confirm('Start sending to up to ' + cap + ' contributors?')) return;

  const res = await fetch('/api/bulk_start', {method:'POST', headers:{'Content-Type':'application/json'},
    body: JSON.stringify({csv, cap, template_name: templateName, media_type: mediaType, media_url: mediaUrl})});
